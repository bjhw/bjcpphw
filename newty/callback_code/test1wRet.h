#ifndef TEST_1_W_RET_H
#define TEST_1_W_RET_H
//======================================================================
// test1wRet.h
//
//
...
public:
  T operator()(T)
  {
    cout << "FunctionObject::operator()(T)" << endl;
    return T();
...
public:
  ...
  Members(T (*functionPtr)(T)):
    functionPtr_(functionPtr)
  {}
  
  T function(T)
  {
    cout << "Members::function(T)" << endl;
    return T();
...
{
public:
  test(Members<T>& members, T (functionPtr)(T)):
  nil_(),
  function_(make_callback((Callback1wRet<T,T>*)0, functionPtr)),
  functionObject_(
		  make_callback
		  (
		   (Callback1wRet<T,T>*)0, 
...
,
  ...
  void invoke_test()
  {
    ...
    member_function_(0);
#ifndef __GNUG__
    member_functionPointer_(0);
    member_functionObject_(0);
#endif
  }



  static void isEqual(const Callback1wRet<T,T>& cb1,
		 const Callback1wRet<T,T>& cb2)
  {
    if(cb1 == cb2)
      cout << ".";
    else
  ...
  static void notEqual(const Callback1wRet<T,T>& cb1,
		 const Callback1wRet<T,T>& cb2)
  {
    if(cb1 == cb2)
      {
  ...
  static void isEqual_lt(const Callback1wRet<T,T>& cb1,
		 const Callback1wRet<T,T>& cb2)
  {
    if((cb1 < cb2) || (cb2 < cb1))
      {
  ...
  static void notEqual_lt(const Callback1wRet<T,T>& cb1,
		 const Callback1wRet<T,T>& cb2)
  {
    if((cb1 < cb2) || (cb2 < cb1))
      cout << ".";
  ...
  void equality_tests()
  {
    Callback1wRet<T,T> nil_copy(nil_);
    Callback1wRet<T,T> function_copy(function_);
    Callback1wRet<T,T> functionObject_copy(functionObject_);
    Callback1wRet<T,T> member_function_copy(member_function_);
#ifndef __GNUG__
    Callback1wRet<T,T> 
...
private:
  Callback1wRet<T,T> nil_;
  Callback1wRet<T,T> function_;
  Callback1wRet<T,T> functionObject_;
  Callback1wRet<T,T> member_function_;

#ifndef __GNUG__
  Callback1wRet<T,T> member_functionPointer_;
  Callback1wRet<T,T> member_functionObject_;
#endif
