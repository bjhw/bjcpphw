...
{
public:
  test(Members<T>& members, void (functionPtr)(T)):
  nil_(),
  function_(make_callback((Callback1<T>*)0, functionPtr)),
  functionObject_(
		  make_callback
		  (
		   (Callback1<T>*)0, 
...
,
  ...
  static void isEqual(const Callback1<T>& cb1,
		 const Callback1<T>& cb2)
  {
    if(cb1 == cb2)
      cout << ".";
    else
  ...
  static void notEqual(const Callback1<T>& cb1,
		 const Callback1<T>& cb2)
  {
    if(cb1 == cb2)
      {
  ...
  static void isEqual_lt(const Callback1<T>& cb1,
		 const Callback1<T>& cb2)
  {
    if((cb1 < cb2) || (cb2 < cb1))
      {
  ...
  static void notEqual_lt(const Callback1<T>& cb1,
		 const Callback1<T>& cb2)
  {
    if((cb1 < cb2) || (cb2 < cb1))
      cout << ".";
  ...
  void equality_tests()
  {
    Callback1<T> nil_copy(nil_);
    Callback1<T> function_copy(function_);
    Callback1<T> functionObject_copy(functionObject_);
    Callback1<T> member_function_copy(member_function_);
#ifndef __GNUG__
    Callback1<T> 
...
private:
  Callback1<T> nil_;
  Callback1<T> function_;
  Callback1<T> functionObject_;
  Callback1<T> member_function_;

#ifndef __GNUG__
  Callback1<T> member_functionPointer_;
  Callback1<T> member_functionObject_;
#endif
